...
class RotaractApp {
  ...
  _init() {
    ...
    // Setup all synchronous UI first
    ...
    // Hide loading screen immediately after sync setup
    this.hideLoading();

    // Load data asynchronously in background
    this.loadAllData();

    // Handle initial hash after short delay
    setTimeout(() => {
  ...
  /* ============================================================
     LOAD ALL DATA (Background — non-blocking)
     ============================================================ */
  async loadAllData() {
    try {
    ...
    // Load everything in parallel, never let one failure block others
    const tasks = [
  ...
  /* ============================================================
     ...
     ============================================================ */
  hideLoading() {
    if (window._loadingFallback) {
      clearTimeout(window._loadingFallback);
    }
  ...
  async loadClubSettings() {
    ...
    if (data) {
      data.forEach(s => { this.clubSettings[s.key] = s.value; });
    }
  }
  ...
  updateDynamicContent() {
    try {
      ...
      // Logos
      const logoEls = document.querySelectorAll('#nav-logo-img, #hero-logo, .about-logo, .contact-logo');
      logoEls.forEach(img => { if (img) img.src = logoColour; });
      ...
      // Social links
      const socialLinks = document.querySelectorAll('a[href*="rotaractdrngpasc"]');
      socialLinks.forEach(link => {
        const href = link.getAttribute('href') || '';
        const platforms = ['instagram', 'facebook', 'linkedin', 'twitter', 'youtube'];
        platforms.forEach(p => {
          if (href.includes(p)) {
            link.href = `https://${p}.com/${socialHandle}`;
          }
        });
      });
      ...
      <div class="upcoming-card-body">
        ...
        <div class="upcoming-card-meta">
          ...
          <div class="upcoming-meta-item">
            ...
            <span>${DateUtils.formatTime(event.start_time)}${event.end_time ? ' – ' + DateUtils.formatTime(event.end_time) : ''}</span>
          </div>
          <div class="upcoming-meta-item">
            <i data-lucide="map-pin"></i>
            <span>${StringUtils.truncate(StringUtils.sanitize(event.venue), 35)}</span>
          </div>
      ...
      <div class="project-card-image-wrap">
        ...
        <div class="project-card-overlay">
          <div class="project-card-overlay-btn">
            <i data-lucide="eye"></i><span>View Details</span>
          </div>
        </div>
  ...
  async openProjectModal(eventId) {
    ...
    if (!event) {
      event = await this.fetchEventById(eventId);
    }

    if (!event) {
      this.showToast('Could not load project details', 'error');
      return;
    }
  ...
  async loadModalPhotos(event) {
    ...
    if (event.poster_urls && Array.isArray(event.poster_urls)) {
      event.poster_urls.forEach(url => {
        if (url && !posterPhotos.includes(url)) posterPhotos.push(url);
      });
    }

    photos.forEach(p => {
      if (!p.is_action_photo && p.photo_url && !posterPhotos.includes(p.photo_url)) {
        posterPhotos.push(p.photo_url);
      }
      if (p.is_action_photo && p.photo_url) {
        actionPhotos.push(p.photo_url);
      }
    });
    ...
    if (posterWrapper && posterPhotos.length > 0) {
      ...
      if (posterPhotos.length > 1) {
        this.modalSwiperInstance = new Swiper('#modal-poster-swiper', {
          ...
          navigation: {
            prevEl: '#modal-poster-swiper .swiper-button-prev',
            nextEl: '#modal-poster-swiper .swiper-button-next'
          }
        });
      }
    } else {
  ...
  shareEvent(event) {
    ...
    if (navigator.share) {
      ...
      navigator.clipboard?.writeText(`${text}\n${url}`).then(() => {
        ...
        this.showToast('Could not share event', 'error');
      });
    }
  }
  ...
  createTimelineCol(leaders, side) {
    ...
    leaders.forEach(leader => {
      ...
      const photoHtml = leader.photo_url
        ? `<img src="${StringUtils.sanitize(leader.photo_url)}"
    ...
    card.innerHTML = `
      ...
      <div class="member-photo-wrap">
        ${member.professional_photo_url
          ? `<img src="${StringUtils.sanitize(member.professional_photo_url)}"
  ...
  setupMemberSearch() {
    ...
    if (input) {
      input.addEventListener('input', (e) => this.searchDebounced(e.target.value));
    }
  }
  ...
  filterMembers(searchTerm = '', bloodFilter = null) {
    ...
    if (bloodFilter === 'board') {
      filtered = filtered.filter(m => m.is_board_member);
    } else if (bloodFilter !== 'all') {
      filtered = filtered.filter(m => m.blood_group === bloodFilter);
    }
  ...
  renderNewsletters(newsletters) {
    ...
    wrapper.innerHTML = newsletters.map(nl => `
          ...
          <div class="newsletter-cover-wrap">
            ${nl.cover_image_url
              ? `<img src="${StringUtils.sanitize(nl.cover_image_url)}"
                       alt="${StringUtils.sanitize(nl.title)}"
                       class="newsletter-cover"
                       loading="lazy"
                       onerror="this.parentElement.innerHTML='<div class=\\'newsletter-cover-placeholder\\'><i data-lucide=\\'newspaper\\'></i></div>'" />`
              : `<div class="newsletter-cover-placeholder"><i data-lucide="newspaper"></i></div>`}
          </div>
          <div class="newsletter-body">
            <div class="newsletter-month">${StringUtils.sanitize(nl.month)} ${nl.year}</div>
            <div class="newsletter-title">${StringUtils.sanitize(nl.title)}</div>
            ${nl.description ? `<p class="newsletter-desc">${StringUtils.sanitize(nl.description)}</p>` : ''}
            ${nl.pdf_url
              ? `<a href="${StringUtils.sanitize(nl.pdf_url)}" target="_blank" rel="noopener noreferrer"
                     class="btn btn-primary btn-sm" style="margin-top:auto;" onclick="event.stopPropagation()">
  ...
  updateLightboxImage() {
    ...
    if (caption) {
      caption.textContent = `Photo ${this.lightboxIndex + 1} of ${this.lightboxImages.length}`;
    }
  }
  ...
  setupJoinForm() {
    ...
    if (photoInput && photoLabel) {
      photoInput.addEventListener('change', (e) => {
        ...
        if (!Validate.fileSize(file, FILE_LIMITS.APPLICATION_PHOTO)) {
          this.showToast('Photo must be under 5MB', 'error');
          photoInput.value = '';
          return;
        }
    ...
    form?.addEventListener('submit', async (e) => {
      e.preventDefault();
      await this.submitJoinForm(form);
    });
  }
  ...
  async submitJoinForm(form) {
    ...
    if (btn) {
      btn.disabled = true;
      btn.innerHTML = '<i data-lucide="loader-2"></i><span>Submitting...</span>';
      try { lucide.createIcons(); } catch (e) {}
    }
    ...
    try {
      ...
      try {
        ...
        if (!uploadError && uploadData) {
          photoUrl = ImageUtils.getPublicUrl(STORAGE_BUCKETS.APPLICATIONS, uploadData.path);
        }
      ...
      form.reset();
      if (document.getElementById('join-photo-label')) {
        document.getElementById('join-photo-label').textContent = 'Click to upload photo (Max 5MB)';
      }
      ...
      if (btn) {
        btn.disabled = false;
        btn.innerHTML = '<i data-lucide="send"></i><span>Submit Application</span>';
        try { lucide.createIcons(); } catch (e) {}
      }
    }
  }
  ...
  setupBloodRequestForm() {
    ...
    form.addEventListener('submit', async (e) => {
      e.preventDefault();
      await this.submitBloodRequest(form);
    });
  }
  ...
  async submitBloodRequest(form) {
    ...
    try {
      ...
      if (btn) {
        btn.disabled = false;
        btn.innerHTML = '<i data-lucide="send"></i><span>Submit Blood Request</span>';
        try { lucide.createIcons(); } catch (e) {}
      }
    }
  }
  ...
  sendBloodRequestWhatsApp(data) {
    ...
    [num1, num2].forEach(num => {
      ...
      window.open(`https://api.whatsapp.com/send?phone=91${clean}&text=${message}`, '_blank');
    });
  }
  ...
  setupRealtimeSubscriptions() {
    try {
      this.db
        ...
        .on('postgres_changes', {
          event: '*', schema: 'public', table: 'events', filter: 'status=eq.approved'
        }, async () => {
...
document.addEventListener('DOMContentLoaded', () => {
  try {
    ...
    // Still hide loading screen even if app fails
    const screen = document.getElementById('loading-screen');
